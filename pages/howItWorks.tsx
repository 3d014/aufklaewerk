...
const HowItWorks = () => {
  ...
  return <Box sx={{dispaly:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
  ...
  <Box sx={classes.sixStepsWrapper}>
    ...
    <Grid container spacing={2} sx={classes.sixStepsInnerBox}>
      <Grid item xs={12} md={6}>{t("step1")}</Grid>
      <Grid item xs={12} md={6}>{t("step2")}</Grid>
      <Grid item xs={12} md={6}>{t("step3")}</Grid>
      <Grid item xs={12} md={6}>{t("step4")}</Grid>
      <Grid item xs={12} md={6}>{t("step5")} </Grid>
      <Grid item xs={12} md={6}>{t("step6")}</Grid>
    </Grid>
  </Box></Box>
  ...
  <Box sx={classes.endingQuestionBox}>
    <p>{t("question")}</p>
    <div>
      {t("preLinkText")} <Link href="/contact" style={classes.endingQuestionLinkWord}>{t("linkWord")}</Link></div>
